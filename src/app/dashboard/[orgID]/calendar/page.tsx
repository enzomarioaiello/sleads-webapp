...
import { CalendarHeader } from "../projects/[projectId]/calendar/components/CalendarHeader";
import { CalendarFilters } from "../projects/[projectId]/calendar/components/CalendarFilters";
import { CalendarForm } from "../projects/[projectId]/calendar/components/CalendarForm";
import { CalendarView } from "../projects/[projectId]/calendar/components/CalendarView";
import {
  AgendaItem,
  AgendaFormData,
  FilterType,
  ViewType,
} from "../projects/[projectId]/calendar/components/types";
import {
  formatLocalDate,
  formatLocalTime,
} from "../projects/[projectId]/calendar/components/utils";
...
export default function OrganizationCalendarPage() {
  ...
  // Get all agenda items from all projects in the organization
  const agendaItems = useQuery(api.projectAgenda.getAgendaItems, {
    organizationId: orgId,
    // Don't pass projectId to get all events from all projects
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Require project selection for new items
      if (!editingItemId && !selectedProjectId) {
        toast({
          title: t("dashboard_internal.project_detail.calendar.error"),
          description: "Please select a project for this event.",
          variant: "error",
        });
        setIsSubmitting(false);
        return;
      ...
      const projectId =
        editingItemId && agendaItems
          ? agendaItems.find((item) => item._id === editingItemId)?.projectId
          : selectedProjectId;
  ...
  if (projects === undefined || agendaItems === undefined) {
    return (
  ...
  if (projects.length === 0) {
    return (
      <div className="space-y-6">
        <div className="bg-white dark:bg-sleads-slate900 rounded-xl border border-slate-200 dark:border-sleads-slate800 p-8 text-center">
          <p className="text-slate-600 dark:text-sleads-slate400">
            No projects found. Please create a project first to use the
            calendar.
          </p>
          <Link
            href={`/dashboard/${orgId}/projects`}
            className="mt-4 inline-block text-sleads-blue hover:underline"
          >
            Go to Projects
          </Link>
        </div>
